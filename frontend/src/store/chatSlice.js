...
//       // В extraReducers для fetchChannels.fulfilled
//       .addCase(fetchChannels.fulfilled, (state, action) => {
//         state.channels = action.payload;
//         state.loading = false;

//         // Автоматически выбираем General или Random канал при первой загрузке
//         const generalChannel = action.payload.find(ch => ch.name === 'General');
//         const randomChannel = action.payload.find(ch => ch.name === 'Random');

//         state.currentChannelId = generalChannel?.id || randomChannel?.id || action.payload[0]?.id || null;
//       });
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(fetchChannels.fulfilled, (state, action) => {
        ...
        // Автовыбор канала General или первого доступного
